...
class createRatingQuestion {
  ...
  start() {
    ...
    if (this.updateVersionChrome > chrome.runtime.getManifest().version) {
      setTimeout(() => {
          const { close, header, content } = this.createDialog();
          header.textContent = `Skip Video Ads in YouTube™ v ${chrome.runtime.getManifest().version} available on the new version(${this.updateVersionChrome}). Update it`;
...
async function info(adBlockSelector) {
  ...
  try{
    const filters = [
        ...
        new createRatingQuestion(options.updateVersionChrome, options.isRateUsPopupEnabled, options.videoCount, options.nextRatingRequest, options.isMessage, options.isMessageURL)
    ];
