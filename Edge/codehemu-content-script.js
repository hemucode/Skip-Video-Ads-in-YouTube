...
class createRatingQuestion {
  ...
  start() {
    ...
    if (this.updateVersionEdge > chrome.runtime.getManifest().version) {
      setTimeout(() => {
          const { close, header, content } = this.createDialog();
          header.textContent = `Skip Video Ads in YouTube™ v ${chrome.runtime.getManifest().version} available on the new version(${this.updateVersionEdge}). Update it`;
      ...
      if (this.isRateUsPopupEnabled && this.nextRatingRequest && this.videoCount > this.nextRatingRequest) {
        ...
        setTimeout(() => {
            ...
            const button = this.createElement(content,  WEBSTORE_REVIEW,`❤️ ${chrome.i18n.getMessage("helpUsWithAReview")}`);
            button.addEventListener("click", () => {
              chrome.storage.sync.set({
            ...
            const button2 = this.createElement(content, "",`💨  ${chrome.i18n.getMessage("later")}`);
            ...
            const button3 = this.createElement(content, "", `👎  ${chrome.i18n.getMessage("dontAskAgain")}`);
...
async function info(adBlockSelector) {
  ...
  try{
    const filters = [
        ...
        new createRatingQuestion(options.updateVersionEdge, options.isRateUsPopupEnabled, options.videoCount, options.nextRatingRequest, options.isMessage, options.isMessageURL)
    ];
